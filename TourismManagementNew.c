...
void CheckTicket(user *h) {
    ...
    if (!strcmp(h->place, "N/A") || h->price == 0.0 || h->numtick == 0) {
...
void PrintTicket(user *h) {
    ...
    // Check if the user has a booked ticket
    if (!strcmp(currentUser->place, "N/A") || currentUser->price == 0.0 || currentUser->numtick == 0) {
        printf("You do not have a ticket booked yet\n");
        return;
    }

    // Calculate the total cost
    float total = currentUser->price * currentUser->numtick;

    // Create the file name based on the user's username
    char filename[150];
    snprintf(filename, sizeof(filename), "%s_ticket.txt", currentUser->username);
    ...
    fprintf(fp, "Email ID: %s\nTour Code: %s\nTicket Cost: Rs %.2f\nNumber of tickets: %d\nTotal Cost: Rs %.2f\n", 
            currentUser->username, currentUser->place, currentUser->price, currentUser->numtick, total);

    // Close the file
    fclose(fp);

    printf("Ticket printed successfully to %s\n", filename);
}
...
void LogoutUser() {
    if (currentstate == menu || strcmp(currentuser, "") == 0) {
